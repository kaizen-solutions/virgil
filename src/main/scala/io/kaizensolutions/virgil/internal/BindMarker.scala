...
private[virgil] trait BindMarker { self =>
  ...
  def value: ScalaType
  def write: CqlEncoder[ScalaType]

  override def toString: String =
    s"Column(name = ${name.name}, value = $value)"
}
private[virgil] object BindMarker {
  def make[A](columnName: BindMarkerName, columnValue: A)(implicit evidence: CqlEncoder[A]): BindMarker =
    new BindMarker {
      type ScalaType = A
      def name: BindMarkerName = columnName
      def value: A             = columnValue
      def write: CqlEncoder[A] = evidence
    }
  ...
  def from(valueInCql: ValueInCql, columnName: BindMarkerName): BindMarker =
    new BindMarker {
      ...
      def write: CqlEncoder[ScalaType] = valueInCql.writer
    }
  ...
  def withName(columnName: BindMarkerName, existing: BindMarker): BindMarker = new BindMarker {
    ...
    override def write: CqlEncoder[ScalaType] = existing.write
  }
}
