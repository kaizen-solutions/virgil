...
final case class CqlInterpolatedString private (
  ...
  def query[Output](implicit evidence: CqlDecoder[Output]): CQL[Output] =
    CQL.cqlQuery(queryString, BindMarkers.from(dataToBeBound))

  def query: CQL[Row] =
    CQL.cqlQuery(queryString, BindMarkers.from(dataToBeBound))(CqlDecoder.cqlDecoderForCqlColumnDecoder[Row])
