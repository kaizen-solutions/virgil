...
import io.kaizensolutions.virgil.codecs.{Reader, RowReader}
...
final case class CqlInterpolatedString private (
  ...
  def query[Output](implicit evidence: RowReader[Output]): CQL[Output] =
    CQL.cqlQuery(queryString, BindMarkers.from(dataToBeBound))

  def query: CQL[Row] =
    CQL.cqlQuery(queryString, BindMarkers.from(dataToBeBound))(Reader.rowReader)
