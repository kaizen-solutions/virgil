...
trait RelationSyntax {
  ...
  implicit class RelationOps(rawColumn: String) {
    ...
    def ===[A](value: A)(implicit ev: CqlEncoder[A]): Relation.Binary[A] =
    ...
    def like[A](value: A)(implicit ev: CqlEncoder[A]): Relation.Binary[A] =
      Relation.Binary(column, BinaryOperator.Like, value, ev)

    def in[A](value: A)(implicit ev: CqlEncoder[A]): Relation.Binary[A] =
